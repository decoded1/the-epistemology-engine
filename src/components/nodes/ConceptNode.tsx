...
import { ConceptNodeData, ConceptNode as ConceptNodeType } from '../../types';
...
export function ConceptNode({ id, data, selected, dragging }: NodeProps<ConceptNodeType>) {
    const updateNodeData = useGraphStore(state => state.updateNodeData);

    // Circular custom port styles
    const handleClass = `!w-[8px] !h-[8px] !bg-bg-elevated !border-[1.5px] rounded-full transition-all z-10 hover:!scale-[1.4] hover:!bg-accent-blue hover:!border-accent-blue hover:!shadow-[0_0_8px_rgba(104,137,255,0.5)] cursor-crosshair ${selected ? '!border-accent-blue' : '!border-text-dim'}`;
    ...
    return (
        <div className={`w-[300px] bg-bg-surface border rounded-xl transition-all duration-180 relative group/node select-none
            ${selected
                ? 'border-accent-blue-border shadow-[0_8px_24px_rgba(0,0,0,0.4),0_0_0_1px_rgba(104,137,255,0.15),0_0_24px_-8px_rgba(104,137,255,0.15)] z-10'
                    ...
                    <div className="px-[14px] pb-[12px] flex flex-col gap-[14px] border-t border-border-base pt-[12px] drawer-content">
                        ...
                        {data.synthesis && (
                            <div className="p-[10px] bg-accent-blue-muted border border-accent-blue-border rounded-lg mb-[2px]">
                                <div className="text-[9px] font-semibold font-mono tracking-[0.1em] uppercase text-accent-blue mb-[6px] opacity-80">
                                    ✦ Synthesis
                                </div>
