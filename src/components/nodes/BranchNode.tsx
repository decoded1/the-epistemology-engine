...
export function BranchNode({ id, data, selected, dragging }: NodeProps<BranchNodeType>) {
    ...
    return (
        ...
        >
            ...
            <NodeHeader id={id} type="branch" title={data.title} description={data.description} onUpdate={(updates) => updateNodeData(id, updates)} />
            ...
            <NodeFooter expanded={!!data.expanded} onToggle={() => updateNodeData(id, { expanded: !data.expanded })} type="branch" referenceCount={totalReferences} hasSynthesis={!!data.synthesis} />
        </div>
    );
}